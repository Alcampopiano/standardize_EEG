function [hPropsPane,parameters, cancelhit] = propertiesGUI(hParent, parameters, filename, selectedBranch)
...
% al hack to collapse all
%model.expandAll();
%model.collapseFirstLevel();
%model.collapseLastLevel();
model.collapseAll();
%model.collapseRow();
...
if nargin == 1 || isempty(hParent) || isequal(hParent,0)
    ...
    %btCancel = uicontrol('String','Cancel', 'Units','pixel', 'Pos',[100,5,60,30], 'Tag','btCancel', 'Callback',@cancelfunc); %#ok<NASGU>
...
if wasFigCreated
    ...
    if getappdata(0,'isParamsGUIApproved')
        ...
        % *******
        % al hack to pass if cancel button was hit to outside function
        cancelhit=0; % al hack
...
pane.setShowDescription(true);  % YMA: we don't currently have textual descriptions of the parameters, so no use showing an empty box that just takes up GUI space...
...
parameters.my_category.and_a_subcategory.is_OK = pi;
...
parameters.cell_array_property  = {'eve1','eve2'};
...
global DES % al hack to pass dynamic descriptions
...
if isstruct(parameters) && ~isempty(parameters)
    ...
    if numParameters > 1
        ...
        %   al_describe{i}=['hello_world', ' ', num2str(i)];
        %end
        ...
        for field_idx = 1 : length(field_names)
            ...
            %% Al hack to pass different descriptions for each study
            
            %%%%%%%%%%%%%%%%%%%%%%%%
            
            % determine which study you are on
            curfields=fieldnames(DES);
            
            findstud=find(strcmp(curfields,field_name)); % gets highest level comparison
            if ~isempty(findstud)
                curstd=curfields{findstud};
                DES.curstd=curstd;
            else
                curstd=DES.curstd;
            end
            
            found=findfield(DES.(curstd),field_name);
            if found
                field_description=DES.(curstd).(field_name);
            else
                 field_description='';
            end
...
%field_label = strrep(field_label,'_',' '); % al hack
...
if prop.isEditable
    ...
    if isempty(origProp)
        ...
        prop.setDisplayName(['<html><font color="red">' label]);
    ...
    %  al hack
    %  Should just be prop.setDisplayName(['<html><font color="gray">' label]);
    %%%%%%%%%%%%%%%%%
    if strfind(label,'Event_labels')
        labpre=['<html><font color="black">' label];
        %labind=strfind(label,'labels');
        labind=strfind(label,'_');
        labind=max(labind); % final underscore
        trailind=label(labind:end); % just grab underscore and number
        %labpost=['<html><font color="red">' trailind];
        labpost=['<html><font size:"0">' trailind]; % invisible somehow. good.
        
        label=[labpre(1:end-size(trailind)-1) labpost];
        %label=['<html><font color="black">' label];        
        prop.setDisplayName(label);
    else
        % Set the property's label to be gray
        prop.setDisplayName(['<html><font color="black"><b>' label]);
        %%%%%%%%%%%%%%%%%
    end
...
if ~isempty(data)
    ...
    for idx = 1 : length(items)
        ...
        if flag
            ...
            %%%%%%% al hack start
            % to see if we have event structure
            par_fields=fieldnames(data);
            atevent=0;
            atsub=0;
            if strfind(items{idx},'event_labels') %&& ~strfind(items{idx},'event_labels')
                atevent=1;
                eveind=idx;
                
             % are we at a "subject" sub structure?   
            elseif strfind(items{idx},'subjects')               
                atsub=1;
                subind=idx;             
            end
            
            % it doesnt seem to ever get here...
            if isfield(cpy,items{idx})
                cpy = cpy.(items{idx});
            elseif atevent==1
                
                sub_field=items{eveind};
                %child_field=items{idx};
                und_ind=strfind(sub_field,'_');
                und_ind=max(und_ind); % get underscore index
                parind=str2double(strrep(sub_field(und_ind:end),'_','')); % isolate number
                propName=[par_fields{parind},'.', propName];
                
            elseif atsub==1;
                
                subject_field=items{idx};
                subject_name=items{idx+1}; %assuming that there are only two fields here
                study_name=subject_name(1:end-5);  
                propName=[study_name, '.', subject_field, '.',subject_name];
                %propName=[par_fields{parind},'.', propName];
            end
            %%%%%%% al hack end
            
            %%%%%%% this is the code that was here originally
            % if isfield(cpy,items{idx})
            %
            %     cpy = cpy.(items{idx});
            % else
...
for propsIdx = 1 : length(propsArray)
    ...
    if ~isOk || isInit,  okEnabled = 'off';  end % al hack - should be 'off'
end

% Update the <OK> button's editability state accordingly
btOK = findall(hContainer, 'Tag','btOK');
set(btOK, 'Enable',okEnabled); % AL HACK should be okEnabled, not 'on'
